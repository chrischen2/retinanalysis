...
def plot_mosaics_for_datasets(df_exp_search: pd.DataFrame,
    ...
    kwargs are fed into plot_rf function.
    ...
    ss_version (str): Kilosort version subdir to load. Default None resolves it
    per chunk with :func:`retinanalysis.analyze.detect_ss_version`, which walks
    every configured volume and picks by ``SS_VERSION_PRIORITY`` — ``kilosort2.5``
    when present, otherwise ``kilosort2``. Pass a fixed version only when you
    know every row shares it.
    ...
    for e_idx, exp in enumerate(exp_names):
        ...
        # chunk_name column with use_chunk_column=True.
        if has_chunk_col and chunk_names[e_idx]:
            candidate_chunks = [chunk_names[e_idx]]
        else:
            exp_summary = get_exp_summary(exp)
            assert exp_summary is not None
            noise_protocol_name = get_noise_name_by_exp(exp)
            candidate_chunks, _ = get_noise_chunks_sorted_by_distance(
                exp_summary, datafile_names[e_idx],
                noise_protocol_name = noise_protocol_name,
                verbose = True)

        analysis_chunk = None
        for candidate in candidate_chunks:
            # find_path walks the volumes in read-priority order (local SSDs,
            # then the NAS), so a chunk that never made it onto the local
            # mirror still resolves off the server.
            chunk_dir = find_path('analysis', exp, candidate)
            if not chunk_dir or not os.path.isdir(chunk_dir):
                print(f'{exp}/{candidate}: no analysis dir on any configured '
                      f'volume, skipping.')
                continue
            try:
                # Auto-detect unless the caller pinned a version: older dates
                # only have kilosort2, so the kilosort2.5 default would miss.
                version = ss_version or detect_ss_version(
                    exp, chunk_name = candidate, kind = 'analysis')
                ...
                print(f'Could not use chunk {candidate}. Error: {e}')

        if analysis_chunk is None:
            # One unusable date should not abort the sweep over the rest.
            print(f'Could not load any noise chunk for {exp}, skipping.\n')
            continue
...
def get_noise_chunks_sorted_by_distance(df_exp_summary: pd.DataFrame, datafile_name: str,
    ...
    prefer_preceding (bool): Rank noise chunks recorded *before* the protocol
    ahead of any recorded after it, each group ordered nearest-first. This is
    the default because the preceding chunk is the one the retina was in when
    the protocol ran — a chunk recorded afterwards has an intervening protocol's
    worth of adaptation and drift between it and the data being typed, even when
    ...
    noise_chunk_distances List[int]: gap in minutes for each listed noise chunk,
    ...
    for chunk_name in noise_chunk_names:
        ...
        noise_chunk_precedes.append(precedes)

    noise_chunk_distances = np.asarray(noise_chunk_distances, dtype=float)
    noise_chunk_precedes = np.asarray(noise_chunk_precedes, dtype=bool)
    if prefer_preceding:
        # lexsort orders by the LAST key first: group on "came after" (False
        # sorts first), then by gap within each group.
        order = np.lexsort((noise_chunk_distances, ~noise_chunk_precedes))
    else:
        order = np.argsort(noise_chunk_distances)

    noise_chunk_names = noise_chunk_names[order]
    noise_chunk_distances = noise_chunk_distances[order]
    noise_chunk_precedes = noise_chunk_precedes[order]

    if verbose:
        print(f'Found {len(noise_chunk_names)} noise chunks for protocol "{noise_protocol_name}"')
        print('Noise chunks in preference order'
              + (' (preceding first):' if prefer_preceding else ' (by distance):'))
        for chunk_name, distance, precedes in zip(
                noise_chunk_names, noise_chunk_distances, noise_chunk_precedes):
            print(f'  {chunk_name}: {distance:.2f} minutes '
                  f'{"before" if precedes else "after"} the protocol')
